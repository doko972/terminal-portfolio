<x-app-layout>
    <x-slot name="header">
        <div class="terminal-header">
            <h2>
                <span class="prompt">root@portfolio:~$</span> nano new_project.json
            </h2>
        </div>
    </x-slot>
                ...
                <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group full-width">
                            ...
                            <input type="text" name="title" id="title"
                                class="terminal-input @error('title') error @enderror" value="{{ old('title') }}"
                                required placeholder="Ex: Portfolio Personnel">
                            @error('title')
                        ...
                        {{-- <div class="form-group full-width">
                            <label for="image" class="form-label">
                                <span class="prompt">></span> Image / Screenshot
                            </label>
                            <input type="file" 
                                   name="image" 
                                   id="image" 
                        ...
                        <div class="form-group full-width">
                            <label for="images" class="form-label">
                                <span class="prompt">></span> Images du projet (max 10)
                            </label>
                            <input type="file" name="images[]" id="images"
                                class="terminal-input-file @error('images') error @enderror" accept="image/*" multiple
                                onchange="previewImages(event)">
                            <p class="help-text">Format accepté : JPG, PNG, GIF, WEBP (max 2Mo par image). La première
                                image sera l'image principale.</p>
                            @error('images')
                                <p class="error-message">{{ $message }}</p>
                            @enderror
                            @error('images.*')
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" name="url" id="url"
                                class="terminal-input @error('url') error @enderror" value="{{ old('url') }}"
                        ...
                        <div class="form-group">
                            ...
                            <input type="url" name="github_url" id="github_url"
                                ...
                                value="{{ old('github_url') }}" placeholder="https://github.com/username/projet">
                            @error('github_url')
                                <p class="error-message">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="termine" {{ old('status') == 'termine' ? 'selected' : '' }}>Terminé
                                </option>
                                <option value="en_cours" {{ old('status') == 'en_cours' ? 'selected' : '' }}>En cours
                                </option>
                                <option value="archive" {{ old('status') == 'archive' ? 'selected' : '' }}>Archivé
                                </option>
                            </select>
                            @error('status')
                                <p class="error-message">{{ $message }}</p>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="order" id="order"
                                class="terminal-input @error('order') error @enderror" value="{{ old('order', 0) }}"
                                min="0" placeholder="0">
                    ...
                    <div class="form-actions">
                        <button type="submit" class="terminal-btn">
                            <span class="prompt">✓</span> Créer le projet
                        </button>
                        <a href="{{ route('admin.projects.index') }}" class="terminal-btn-secondary">
                            Annuler
                        </a>
